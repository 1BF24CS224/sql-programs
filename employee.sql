...
FROM Employee M
JOIN Employee E ON M.EmpNo = E.MgrNo
GROUP BY M.EmpNo
HAVING M.Sal > AVG(E.Sal);

WITH ManagerHierarchy AS (
    SELECT E.DeptNo, E.EmpNo, E.EName, E.MgrNo, E.HireDate,
           ROW_NUMBER() OVER (PARTITION BY E.DeptNo ORDER BY E.HireDate) AS RowNum
    FROM Employee E
    WHERE E.MgrNo IS NOT NULL
)
SELECT DeptNo, EName AS SecondTopManager
FROM ManagerHierarchy
WHERE RowNum = 2;

SELECT E.EmpNo, E.EName, E.Sal, I.IncentiveAmount, I.IncentiveDate
FROM Employee E
JOIN Incentives I ON E.EmpNo = I.EmpNo
WHERE I.IncentiveDate = '2019-01-01'
AND I.IncentiveAmount = (
    SELECT MAX(IncentiveAmount)
    FROM Incentives
    WHERE IncentiveDate = '2019-01-01'
    AND IncentiveAmount < (
        SELECT MAX(IncentiveAmount)
        FROM Incentives
        WHERE IncentiveDate = '2019-01-01'
...
SELECT E.EmpNo, E.EName, D.DName AS DeptName
FROM Employee E
JOIN Dept D ON E.DeptNo = D.DeptNo
